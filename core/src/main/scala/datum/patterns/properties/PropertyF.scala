...
case class IntPropF(value: Int) extends PropertyF[Nothing]
case class StringPropF(value: String) extends PropertyF[Nothing]
case class CollectionPropertyF[R](properties: SortedMap[String, R]) extends PropertyF[R]
  ...
  implicit val traverse: Traverse[PropertyF] = new DefaultTraverse[PropertyF] {
    override def traverse[G[_], A, B](fa: PropertyF[A])(f: A => G[B])(implicit G: Applicative[G]): G[PropertyF[B]] = {
      fa match {
        ...
        case p @ IntPropF(_)    => G.pure(p)
        case p @ StringPropF(_) => G.pure(p)
        case CollectionPropertyF(properties) =>
          val traversed = Traverse[SortedMap[String, ?]].traverse(properties)(f)
          G.map(traversed)(CollectionPropertyF.apply)
      }
    }
  }
}
